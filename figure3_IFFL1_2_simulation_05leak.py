...
###############################################################################
'''
lf = [0.025, 0.025, 0.025, 0.025, 0.025, 0.025, 0.025, 0.025, 0.025]
# Defining network
...
''' simulating the IFFL'''
IFFL12.simulate(t_vec1,1,leak=lf)
...
plt.subplot(3,4,9)
plt.plot(sim_t/3600,G5/G_tot[4],color=[0,0,1],linewidth=4)  
...
# Define initial conditions
#                  dA1  dA2  dA3  dA4  dA5  dA6
dA_tot = np.array([0, 125, 250, 250, 250, 250]) # total activator added

#                 G1  G2 G3  G4  G5  G6 G7  G8  G9
G_tot = np.array([25, 5, 25, 15, 25, 25, 5, 25, 25]) # total genelet added
...
G1 = IFFL12.output_concentration['GdA1']
G3 = IFFL12.output_concentration['GdA3']
G6 = IFFL12.output_concentration['GdA6']
G5 = IFFL12.output_concentration['GdA5']
G9 = IFFL12.output_concentration['GdA9']
sim_t = IFFL12.sol.t
...
plt.figure(3)
plt.subplot(3,4,1)
plt.plot(sim_t/3600,G1/G_tot[0],color=[0,0.8,0],linewidth=4,linestyle='--')
plt.plot(sim_t/3600,G6/G_tot[5],color=[0,0.5,0],linewidth=4)  
plt.ylabel('Fraction ON',fontsize=fs,weight='bold')
plt.xticks(fontsize=fs,weight='bold')
plt.yticks(fontsize=fs,weight='bold')
ax1 = plt.gca()
...
plt.subplot(3,4,5)
plt.plot(sim_t/3600,G3/G_tot[2],color=[1,0,0],linewidth=4,linestyle='--')  
...
plt.subplot(3,4,9)
plt.plot(sim_t/3600,G5/G_tot[4],color=[0,0,1],linewidth=4,linestyle='--')  
plt.plot(sim_t/3600,G9/G_tot[8],color=[0.4,0,0.4],linewidth=4,linestyle='--')  
plt.ylabel('Fraction ON',fontsize=fs,weight='bold')
plt.xlabel('time (hr)',fontsize=fs,weight='bold')
plt.xticks(fontsize=fs,weight='bold')
plt.yticks(fontsize=fs,weight='bold')
